...
namespace jobWebSite.Controllers
{
    ...
    public class AccountController : Controller
    {
        ...
        {
            ViewBag.UserType = new SelectList(db.Roles.Where(a => !a.Name.Contains("Adminstrators") && !a.Name.Contains("Admins")).ToList(), "Name", "Name");
            return View();
        }
        ...
        public async Task<ActionResult> Register(RegisterViewModel model)
        {
            if (ModelState.IsValid)
            {
                // ضفتها عمود جديد في ايدنتتي وخليتو ياخذ الرول من جدول الرول
                ViewBag.UserType = new SelectList(db.Roles.Where(a => !a.Name.Contains("Adminstrators") && !a.Name.Contains("Admins")).ToList(), "Name", "Name");
                ...
                if (result.Succeeded)
                {
                    ...
                    // إضافة اليوزر إلى الرول تتم العملية بنفسي أنا
                    // حياخد الاي دي وياخد الرول اللي أنا سجلتها في الرجيستر
                    // For more information on how to enable account confirmation and password reset please visit https://go.microsoft.com/fwlink/?LinkID=320771
                    // Send an email with this link
                    // string code = await UserManager.GenerateEmailConfirmationTokenAsync(user.Id);
                    // var callbackUrl = Url.Action("ConfirmEmail", "Account", new { userId = user.Id, code = code }, protocol: Request.Url.Scheme);
                    // await UserManager.SendEmailAsync(user.Id, "Confirm your account", "Please confirm your account by clicking <a href=\"" + callbackUrl + "\">here</a>");
                    // وحناخذ الرول من المودل اللي اخرته مثلا ناشرون او باحثون وهكذا
                     await UserManager.AddToRoleAsync(user.Id, model.UserType); //حياخذ الا دي من المستخد اللي رح يسجل اللي هو من اليوزر
